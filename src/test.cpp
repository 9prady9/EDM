...
void print_raw_matrix(const double* M, int rows, int cols)
{
  ...
  auto stringVersion = [](double v) { return (v == MISSING_SENTINEL) ? std::string(" . ") : fmt::format("{:.1f}", v); };
...
TEST_CASE("Missing data manifold creation (tau = 1)", "[missingDataManifold]")
{
  ...
  std::vector<double> x = { 11, 12, MISSING_SENTINEL, 14, 15, 16 };
  ...
  SECTION("dt and allowingmissing")
  {
    ...
    std::vector<std::vector<double>> M_true = { { 11.0, MISSING_SENTINEL, 1.5, MISSING_SENTINEL },
                                                { MISSING_SENTINEL, 12.0, 1.5, 0.5 },
                                                { 14.0, MISSING_SENTINEL, 0.5, 1.5 },
                                                { 15.0, 14.0, 1.0, 0.5 } };
...
TEST_CASE("Missing data dt manifold creation (tau = 2)", "[missingDataManifold2]")
{
  ...
  SECTION("Allowing missing values")
  {
    ...
    std::vector<std::vector<double>> M_true = {
      { 11.0, MISSING_SENTINEL, 1.5, MISSING_SENTINEL },
      //  { 12.0, MISSING_SENTINEL, 0.5, MISSING_SENTINEL},
      { MISSING_SENTINEL, 11.0, 1.5, 2.0 },
      { 14.0, 12.0, 0.5, 2.0 },
      { 15.0, MISSING_SENTINEL, 1.0, 2.0 },
      //  { 16.0, 14.0, MISSING_SENTINEL, 1.5},
    };
    std::vector<double> y_true = { 12.0, 14.0, 15.0, 16.0 };
    require_manifolds_match(M, M_true, y_true);
  }
...
TEST_CASE("Wasserstein distance", "[wasserstein]")
{
  ...
  const double NA = MISSING_SENTINEL;
