...
double ManifoldGenerator::calculate_time_increment() const
{
  ...
  for (int i = 1; i < _t.size(); i++) {
    ...
    if (timeDiff <= 0 || _t[i] == MISSING_SENTINEL || _t[i - 1] == MISSING_SENTINEL) {
      continue;
    }
...
void ManifoldGenerator::setup_observation_numbers()
{
  if (!_dt) {
    ...
    for (int i = 0; i < _t.size(); i++) {
      if (_t[i] != MISSING_SENTINEL &&
          (_allow_missing || (_x[i] != MISSING_SENTINEL))) { // TODO: What about co_x missing here?
                                      ...
                                      double& target) const
{
  ...
  if (_dt) {
    double tPred = (targetIndex >= 0) ? _t[targetIndex] : MISSING_SENTINEL;

    for (int j = 0; j < E_dt(E); j++) {
      double tNow = lookup_vec(_t, j);
      if (j == 0 || _reldt) {
        if (tNow != MISSING_SENTINEL && tPred != MISSING_SENTINEL) {
          point[E + j] = dtWeight * (tPred - tNow);
        } else {
          point[E + j] = MISSING_SENTINEL;
        }
      } else {
        double tNext = lookup_vec(_t, j - 1);
        if (tNext != MISSING_SENTINEL && tNow != MISSING_SENTINEL) {
...
{
  if (targetRequired && target == MISSING_SENTINEL) {
    return false;
  }
