  ...
  for (ST_int i = SF_in1(); i <= SF_in2(); i++) {
    if (SF_ifobs(i)) { // Skip rows according to Stata's 'if'
      if (useEveryRow || filter[r]) {
        // Convert MISSING_SENTINEL back to Stata's missing value
        ST_double value = (data[obs] == MISSING_SENTINEL) ? SV_missval : data[obs];
